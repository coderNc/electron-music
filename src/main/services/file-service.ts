 ...
 * First tries to extract lyrics from audio file metadata,
 * then falls back to .lrc file with same name in same directory
...
export async function readLyrics(audioFilePath: string): Promise<string | null> {
  // First try to extract lyrics from audio file metadata
  const { metadataService } = await import('./metadata-service')
  const embeddedLyrics = await metadataService.extractLyrics(audioFilePath)
  if (embeddedLyrics) {
    return embeddedLyrics
  }

  // Fall back to .lrc file
